...
class CODSTATS(commands.Cog):
    ...
    async def codstats(self, ctx, platform, username):
        ...
        try:
            ...
            if data["data"]["lifetime"]["map"]["scorestreakData"]["nuke"]["properties"] != "N/A":
                embed.add_field(name="Nukes", value=data["data"]["lifetime"]["map"]["scorestreakData"]["nuke"]["properties"], inline=True)
            embeds.append(embed)
            await menu(
