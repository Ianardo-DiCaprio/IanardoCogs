...
class IMDB(commands.Cog):
    ...
    async def movie(self, ctx, *, search):
       ...
       from IMDB"""
        ...
        link = "http://www.omdbapi.com/?apikey={api_key}&t={search}&plot=full".format(api_key=api_key, search=search)
        async with aiohttp.request("http://www.omdbapi.com/?apikey={api_key}&t={search}&plot=full".format(api_key=api_key, search=search)) as request:
            data = request.json()
        try:
            title = data["Title"]
    ...
    async def show(self, ctx, *, search):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get("http://www.omdbapi.com/?apikey={api_key}&t={search}&plot=full").format(
