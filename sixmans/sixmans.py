...
class SixMans(commands.Cog):
    ...
    async def pick_blue(self, ctx, captain):
        try:
            ...
            embed = discord.Embed(
                title="VOID ESPORTS™ 6Mans", description=timed, color=0x00FFFF
            )
            embed.set_thumbnail(
                url="https://cdn.discordapp.com/attachments/648743379252805663/684605565946953744/octopus-1.png"
            )
    ...
    async def display_teams(self, ctx):
        ...
        await ctx.send(embed=embed)
        await ctx.send(player.mention for player in self.game.blue)
        await ctx.send(player.mention for player in self.game.orange)
